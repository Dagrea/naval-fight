...
constructor(props) {
    super(props)
    this.state = {
      yourBoard: Array(10).fill(Array(10).fill(Array())),
      enemyBoard: Array(10).fill(Array(10).fill(Array())),
  ...
  randomShip(size,newBoard) {
    ...
    if (startOfShip < 10 ) {
      if (newBoard[0][startOfShip][0] != null) {newBoard = this.randomShip(size,newBoard);return newBoard}
      ...
      for (let i = 0; i < ship.length; i++) {
        ...
        newBoard[firstChar][secondChar] = ["hide",ship];
      }
      return newBoard
    }
    let firstChar = Number(startOfShip.toString().slice(0,1));
    let secondChar = Number(startOfShip.toString().slice(-1));
    if (newBoard[firstChar][secondChar][0] != null) {newBoard = this.randomShip(size,newBoard);return newBoard}
        ...
        if (ship[i] === 0) {
          for (let j = 0; j < 2; j++) {
            for (let k = 0; k < 2; k++) {
              ...
              if (newBoard[firstChar+j][lastChar+k][0] != null) {
                return false
              }
  ...
  randomPositioning() {
    let newBoard1 = [];
    for (let i = 0; i < 10; i++) {
      newBoard1.push(Array(10).fill(Array()));
    }
    let b = 0;
    for (let i = 0; i < 10; i++) {
    ...
    let newBoard2 = [];
    for (let i = 0; i < 10; i++) {
      newBoard2.push(Array(10).fill(Array()));
    }
    b = 0;
    for (let i = 0; i < 10; i++) {
